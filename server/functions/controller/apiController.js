...
router.get('/searchCurrency', (req, res)=>{
    ...
    axios.get(`https://min-api.cryptocompare.com/data/price?fsym=${coinName}&tsyms=BRL`, config)
    .then(price=>{
        if(price.data.Response === 'Error') res.status(404).send({message:'Moeda não encontrada'})
...
router.get('/historicalData', (req, res)=>{
    ...
    axios.get(`https://min-api.cryptocompare.com/data/v2/histoday?fsym=${coinName}&tsym=BRL&limit=${time}`, config)
    .then(result=>{
